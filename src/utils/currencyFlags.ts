// Comprehensive currency flag mapping
export const CURRENCY_FLAGS: Record<string, string> = {
  ...
  'SOS': '🇸🇴', 'SRD': '🇸🇷', 'STD': '🇸🇹', 'STN': '🇸🇹', 'SYP': '🇸🇾', 'SZL': '🇸🇿', 'THB': '🇹🇭', 'TJS': '🇹🇯',
  'TMT': '🇹🇲', 'TND': '🇹🇳', 'TOP': '🇹🇴', 'TRY': '🇹🇷', 'TTD': '🇹🇹', 'TWD': '🇹🇼', 'TZS': '🇹🇿', 'UAH': '🇺🇦',
  'UGX': '🇺🇬', 'USD': '🇺🇸', 'UYU': '🇺🇾', 'UZS': '🇺🇿', 'VES': '🇻🇪', 'VND': '🇻🇳', 'VUV': '🇻🇺', 'WST': '🇼🇸',
  'XAF': '🇨🇲', 'XAG': '🥈', 'XAU': '🥇', 'XCD': '🇦🇬', 'XDR': '🏛️', 'XOF': '🇸🇳', 'XPD': '⚪', 'XPF': '🇵🇫',
  'XPT': '⚫', 'YER': '🇾🇪', 'ZAR': '🇿🇦', 'ZMW': '🇿🇲', 'ZWL': '🇿🇼'
};
...
export const getCurrencyFlag = (currencyCode: string): string => {
  return CURRENCY_FLAGS[currencyCode?.toUpperCase()] || '🏳️';
};
...
export const hasCurrencyFlag = (currencyCode: string): boolean => {
  return currencyCode?.toUpperCase() in CURRENCY_FLAGS;
};

/**
 * Get all available currency codes that have flag mappings
 * @returns Array of currency codes that have flag mappings
 */
export const getAvailableFlagCurrencies = (): string[] => {
  return Object.keys(CURRENCY_FLAGS);
};